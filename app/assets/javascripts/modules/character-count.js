...
export default class CharacterCount {
  ...
  _init () {
    ...
    CharCount.prototype.attach = function (options) {
      ...
      if (countElements) {
        for (var i = 0, len = countElements.length; i < len; i++) {
          ...
          var rawOverLimitMessage = countElement.getAttribute('data-message-over-limit') || 'You have [x] characters too many';
    ...
    CharCount.prototype.updateCountMessage = function (countElementExtended) {
      ...
      var rawOverLimitMessage = countElementExtended.rawOverLimitMessage;
      ...
      countMessage.innerHTML = remainingNumber < 0 ? rawOverLimitMessage.replace('[x]', displayNumber) : rawUnderLimitMessage.replace('[x]', displayNumber);
